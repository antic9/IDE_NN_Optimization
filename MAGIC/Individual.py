...
# W = tf.placeholder(tf.float32, [10,2])
...
class Indivi:
    ...
    def Eval_Accuracy(self,batch_xs, batch_ys):
        temp = np.array((self.gene[0,0:20]).reshape(10,2))
        temp2 = np.array(self.gene[0,20:22])
        
        out =sess.run(accuracy, feed_dict={x: batch_xs, y_: batch_ys, W: temp, b: temp2 })
    ...
    def Finish(self, best_gene,test_input,test_output):
        print(best_gene)
        temp = np.array((best_gene[0,0:20]).reshape(10,2))
        temp2 = np.array(best_gene[0,20:22])
        # print("--------------------")
        # print(test_input)
        # print(test_output)
        # print(temp.shape)
