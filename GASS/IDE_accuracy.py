...
class IDE_NN:
    def __init__(self,batch_xs, batch_ys, popu, u_CR=0.5,u_F=0.5,p2=0.05,c=0.1):
        global POPULATION 
        POPULATION = popu
    ...
    def Mutation(self,count,ps2,UpdatedIndividual,batch_xs, batch_ys):
        ...
        for i in range(POPULATION):
            ...
            for l in range (len(p)):
                As.append(Population[p[l]].fitness_accuracy)
            As.sort()
            #print("Asfitness=",As)
            Ss = []
            ...
            for o in range (POPULATION):
                if Population[p[o]].fitness_accuracy in Ss:
                    xbetter = Population[p[o]].fitness_accuracy
            ...
            #Accuracy
            fitness = Individual.Indivi.Evaluate2(Population[i],batch_xs,batch_ys)
            # print(POPULATION)
            # print(fitness,new_Individual.fitness_accuracy)
            if (new_Individual.fitness_accuracy > fitness):
    ...
    def Best_Individual(self):
        ...
        Population.sort(key = operator.attrgetter('fitness_accuracy'),reverse = True)
